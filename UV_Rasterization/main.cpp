...
{
	string caseRoot = "D:/MyWork/2nd_Garment_Refitting/data/Manneq/WHip_Hop/Halterneck/";

	string sourcePD = "10";
	string targetPD = "30";
	string sourcePrefix = caseRoot + "PD" + sourcePD + "_G/A_pose/";
	string targetPrefix = caseRoot + "PD" + targetPD + "_G/A_pose/";

	string sourceUVName = sourcePrefix + "A_pose_uv.ply";
	string targetUVName = targetPrefix + "A_pose_uv.ply";
	...
	UVMesh_Sample_Interpolation(sourceUVMesh, targetUVMesh, caseRoot + targetPD + "_from_" + sourcePD + "_Sampling.txt");
}

void Geo_UV_Map()
{
	string caseRoot = "D:/MyWork/Huawei/Data/Dream_Chang_An/Garment/Canonical/weld/";
	string CaseName = "PD35";
...
void BodySeed_Sampling_Based_ColorMask()
{
	string caseRoot = "D:/MyWork/Huawei/Data/SMPL_female_default/Tc/body/Canonical/";
	string CaseName = "body";
	...
	string colormaskName = caseRoot + "test/" + CaseName + "_" + string(imgSizeCase) + "_uvmask_colored_0.png";
	cv::Mat colorMask = cv::imread(colormaskName, cv::IMREAD_COLOR);
	colorMask.convertTo(colorMask, CV_32FC3);

	int pixelStep = 16;

	UVMesh.scaleMesh(img_Size, img_Size, 1.);
	RayIntersection myTracer;
	myTracer.addObj(&UVMesh);

	std::vector<int> InFaceID;
	std::vector<int> InBodyTag;
	std::vector<cv::Vec2f> InFaceAB;

	std::vector<cv::Vec3f> CC_bodyKeys;
	std::vector<cv::Vec3i> CC_bodyColors;

	for (int y = 0; y < img_Size; y+=pixelStep)
	{
		for (int x = 0; x < img_Size; x+=pixelStep)
		{
			cv::Vec3f pColor = colorMask.at<cv::Vec3f>(y, x);
			if (pColor[0] + pColor[1] + pColor[2] > 50)
			{
				cv::Vec3f ori(x, img_Size - y, -10.);
				cv::Vec3f dir(0., 0., 1.);
				RTCHit h = myTracer.rayIntersection(ori, dir);
				int fID = h.primID;
				if (fID < 0)
				...
				else
				{
					...
					if (pColor[1] > 150)
						STag = 1;
					else if (pColor[2] > 150)
						STag = 2;
					else
						STag = 0;
					InBodyTag.push_back(STag);

					cv::Vec3f keyPos = (1. - h.u - h.v) * GeoMesh.verts[GeoMesh.faceInds[fID][0]] + h.u * GeoMesh.verts[GeoMesh.faceInds[fID][1]] + h.v * GeoMesh.verts[GeoMesh.faceInds[fID][2]];
					cv::Vec3i keyCC(0., 0., 0.);
					keyCC[STag] = 255;
					CC_bodyKeys.push_back(keyPos);
					CC_bodyColors.push_back(keyCC);
				} // end for else
			}
		} // end for x
	} // end for y

	int numKeys = int(CC_bodyKeys.size());
	char numKeysCase[8];
	std::snprintf(numKeysCase, sizeof(numKeysCase), "%d", numKeys);

	savePlyFile(caseRoot + "test/" + CaseName + "_" + string(numKeysCase) + "_Key.ply", CC_bodyKeys, CC_bodyColors, std::vector<cv::Vec3i>());

	saveSampleInfo_InFaceAB(caseRoot + "test/" + CaseName + "_" + string(numKeysCase) +  "_KeySample.txt", InFaceID, InFaceAB);
	//----save sample tag
	ofstream keyStream(caseRoot + "test/" + CaseName + "_" + string(numKeysCase) +  "_KeyTag.txt");
	keyStream << "#" << " " << InBodyTag.size() << endl;
	for (int i = 0; i < InBodyTag.size(); i++)
		keyStream << InBodyTag[i] << endl;
...
{
	string caseRoot = "D:/MyWork/Huawei/Data/SMPL_female_default/Tc/short_skirt/";
	string ResCaseName = "PD30";
	int img_Size = 408;
	char imgSizeCase[8];
	std::snprintf(imgSizeCase, sizeof(imgSizeCase), "%d", img_Size);
	...
	//-- render normal
	string motionCase = "/silk_chamuse/0_House_Dancing/";
	string vertCase = "/PD30/PD30_";
	string framePref = caseRoot + motionCase + vertCase;
	
	//string framePref = "D:/MyWork/Huawei/code/Garment_DLSS/Round_3/rst/all_2/tshirt2/silk_chamuse/0_House_Dancing/ro/f/";
	//string framePref = "D:/MyWork/Huawei/code/comparisons/G_rst/tshirt2/0_House_Dancing/old/";
...
void calcTexSize()
{
	std::string F_prefix = "D:/MyWork/Huawei/Data/SMPL_female_default/Tc/short_skirt/";
...
void grab_normal_from_nimg()
{
	...
	std::string uvName = F_prefix + "Canonical/weld/PD10_uv.ply";
	int imgS = 408;
	std::string uvMaskName = F_prefix + "Canonical/weld/test/PD10_408_uvmask.png";
	...
	std::string nimg_root = "D:/MyWork/Huawei/code/comparisons/DDE_rst/short_skirt/0_House_Dancing/";
